...
class CWriter {
 ...
 private:
  ...
  void WriteExportLookup(); // Sandbox
  void WriteFuncIndexLookup(); // Sandbox
  void WriteCallbackAddRemove(); // Sandbox
  void WriteInit();
  void WriteFuncs();
...
void CWriter::WriteFuncTypes() {
  // Sandbox: Initalize and use func_type_structs
  Write(Newline());
  ...
  for (TypeEntry* type : module_->types) {
    ...
    Write(" };", Newline());
    Write("sbx->func_types[", func_type_index,
          "] = wasm_rt_register_func_type(&sbx->func_type_structs, "
          "&sbx->func_type_count, ",
...
void CWriter::WriteElemInitializers() {
  ...
  Write("sbx->", ExternalRef(table->name), "_current_index = offset + ",
        std::to_string(first_unused_elem), ";", Newline());
  Write(CloseBrace(), Newline());
}

// Sandbox: Look inside sbx for exports
void CWriter::WriteExportLookup() {
  Write(Newline(),
        "void* WASM_RT_ADD_PREFIX(lookup_wasm2c_nonfunc_export)(void* sbx_ptr, "
        "const char* name) ",
        OpenBrace());
...
void CWriter::WriteFuncIndexLookup() {
  Write(Newline(),
        "void* WASM_RT_ADD_PREFIX(lookup_wasm2c_func_index)(void* sbx_ptr, u32 "
        "param_count, u32 result_count, wasm_rt_type_t* types) ",
        OpenBrace());
  Write("wasm2c_sandbox_t* sbx = (wasm2c_sandbox_t*) sbx_ptr;", Newline());
  Write(
      "return wasm_rt_register_func_type(&sbx->func_type_structs, "
      "&sbx->func_type_count, param_count, result_count, types);",
      Newline());
  Write(CloseBrace(), Newline());
}
...
void CWriter::WriteCallbackAddRemove() {
  ...
  Write(Newline(),
        "u32 WASM_RT_ADD_PREFIX(add_wasm2c_callback)(void* sbx_ptr, u32 "
        "func_type_idx, void* func_ptr) ",
        OpenBrace());
  {
    Write("wasm2c_sandbox_t* sbx = (wasm2c_sandbox_t*) sbx_ptr;", Newline());
    ...
    Write(OpenBrace());
    {
      Write("if (sbx->", ExternalRef(table->name), ".data[i].func == 0) ",
            OpenBrace());
      {
        Write("sbx->", ExternalRef(table->name), ".data[i]",
              " = (wasm_rt_elem_t){sbx->func_types[func_type_idx], ",
              "(wasm_rt_funcref_t) func_ptr };", Newline());
  ...
  Write(Newline(),
        "void WASM_RT_ADD_PREFIX(remove_wasm2c_callback)(void* sbx_ptr, u32 "
        "callback_idx) ",
        OpenBrace());
  {
    Write("wasm2c_sandbox_t* sbx = (wasm2c_sandbox_t*) sbx_ptr;", Newline());
    Write("sbx->", ExternalRef(table->name), ".data[callback_idx].func = 0;",
          Newline());
  }
  Write(CloseBrace(), Newline());
}
